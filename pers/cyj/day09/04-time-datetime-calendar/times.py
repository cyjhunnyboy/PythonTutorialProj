"""
UTC(世界协调世界): 格林尼治天文时间，世界标准时间，在中国来说是UTC+8
DST(夏令时): 是一种节约能源而人为规定时间制度，在夏季调块1个小时

时间的表示形式：
1、时间戳：以整形或浮点型表示时间的一个以秒为单位的时间间隔，这个时间间隔的基础值是从1970年1月1日零点开始算起
...
# 将时间戳转为字符串
# 相当于：time.asctime(time.localtime(time.time()))
p = time.ctime(c)
print(p)
...
i = time.strftime("%Y-%m-%d %H:%M:%S", b)
print(i)

# 将时间元组转换成给定格式的字符串，参数2为时间元组，如果没有参数2，默认转当前时间
j = time.strftime("%Y-%m-%d %X", b)
print(j)
print(type(j))
